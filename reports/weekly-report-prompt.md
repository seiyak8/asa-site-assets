...
`campaign` 欄に `Uh-oh! You've connected more accounts than your Free plan allows`
というメッセージが入る。

したがって当面、**広告・Instagram のセクションは省略し、
「Windsor.ai の無料プランのため未収録」と明記すること。**

毎回試す必要はないが、もし `get_data` を呼ぶなら上記メッセージが入っていないかを
必ず確認すること。**プラン上限のメッセージを実績ゼロとして報告してはいけない。**
「広告費0円」と「広告費が取得できない」は経営判断上まったく別の意味になる。

有料化するか Meta Graph API に切り替えれば取得できるようになる。方針が決まったら
この節を書き換える。
